...
select jsonb_build_object(
  ...
  'functions', (select jsonb_agg(jsonb_build_object('name', p.proname, 'definition', pg_get_functiondef(p.oid))) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('bootstrap_finance_workspace','is_finance_workspace_member','save_finance_state')),
  'policies', (select jsonb_agg(to_jsonb(p)) from pg_policies p where schemaname='public' and tablename like 'finance_%'),
  'columns', (select jsonb_agg(to_jsonb(c)) from information_schema.columns c where table_schema='public' and table_name like 'finance_%'),
