...
select jsonb_build_object(
  ...
  'tables', jsonb_build_object(
    ...
    'base_client_app_state', (select coalesce(jsonb_agg(to_jsonb(t)), '[]') from public.base_client_app_state t),
    'base_client_actions', (select coalesce(jsonb_agg(to_jsonb(t)), '[]') from public.base_client_actions t),
  ...
  'functions', (select jsonb_agg(jsonb_build_object('name', p.proname, 'definition', pg_get_functiondef(p.oid))) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('bootstrap_finance_workspace','is_finance_workspace_member','save_finance_state','save_base_client','base_client_fields','is_base_workspace_member','load_finance_snapshot','is_not_workspace_member','activate_not_workspace','save_not_service','renew_not_service',
    'is_client_central_manager','is_client_archived_in_app','set_client_app_archived','client_delete_preview','delete_client_from_central',
    '_save_base_client_before_lifecycle','_save_not_service_before_lifecycle','_renew_not_service_before_lifecycle','_save_finance_state_before_lifecycle')),
