...
const db=await makeDatabase();await asUser(db,ids.owner);
...
await db.exec('reset role');await db.query("delete from base_app_members where app_id='not'");await asUser(db,ids.owner);
...
const tables=new Set(['finance_workspaces','finance_workspace_members','base_app_members','base_clients','not_services','not_service_events']);
const rpcArgs={activate_not_workspace:['p_workspace_id','p_name'],save_base_client:['p_workspace_id','p_client_id','p_payload','p_status','p_archived','p_expected_revision'],save_not_service:['p_workspace_id','p_id','p_data','p_expected_revision'],renew_not_service:['p_workspace_id','p_id','p_expected_revision','p_paid_on','p_next_due','p_amount_cents']};
const root=path.resolve('.local-backups/not-ui');
http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(url.pathname.startsWith('/rest/v1/')){
      ...
      const where=[],values=[];for(const [key,value] of url.searchParams)if(['user_id','workspace_id','service_id'].includes(key)&&value.startsWith('eq.')){values.push(value.slice(3));where.push(`${key}=$${values.length}`);}
    ...
    let relative=decodeURIComponent(url.pathname).replace(/^\/not\/?/,'');if(!relative||!path.extname(relative))relative='index.html';
