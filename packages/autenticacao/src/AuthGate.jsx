...
export default function AuthGate({ children, appName = 'Financeiro', mark = 'BF', description = 'Entre para lançar e consultar os dados financeiros da empresa.' }) {
  ...
  if (!isSupabaseConfigured) {
    return <AuthScreen><div className="fx-auth-mark">{mark}</div><p className="fx-auth-eyebrow">BASE {appName}</p><h1>Conexão pendente</h1><p>O aplicativo está pronto para o banco, mas as variáveis do Supabase ainda não foram configuradas neste ambiente.</p></AuthScreen>;
  }
  ...
  if (recoveryMode && session) {
    ...
    return <AuthScreen>
      <div className="fx-auth-mark">{mark}</div><p className="fx-auth-eyebrow">BASE {appName}</p>
  ...
  if (!session) {
    ...
    return <AuthScreen>
      <div className="fx-auth-mark">{mark}</div><p className="fx-auth-eyebrow">BASE {appName}</p>
      <h1>Acesse sua conta</h1><p>{description}</p>
